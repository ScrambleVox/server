...
module.exports = (waveFile, delayTime = 0.1) => {
  ...
  if (bitsPerSample === 8) {
    for (let i = delaySamples; i < waveFile.data.length; i++) {
      let alteredSample = Math.floor(0.75 * (waveFile.data[i - delaySamples] - eightBitZero)) + Math.floor(0.75 * (waveFile.data[i] - eightBitZero)) + eightBitZero;
  ...
  if (bitsPerSample === 16){
    ...
    for (let i = delaySamples; i < waveFile.data.length; i += 2) {
      ...
      let alteredSample = Math.floor(0.75 * (pastSample - sixteenBitZero)) + Math.floor(0.75 * (currentSample - sixteenBitZero)) + sixteenBitZero;
